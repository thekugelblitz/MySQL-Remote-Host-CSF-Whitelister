...
INSTALL_PATH="/root/mysql_csf_whitelist.sh"
CRON_LOG="/var/log/mysql_csf_cron.log"
TAG="Auto-whitelist:mysql"

# Step 1: Generate the actual whitelist script
...
# Step 2: Ensure cronjob exists for every 1 minute
CRON_LINE="* * * * * /bin/bash $INSTALL_PATH >> $CRON_LOG 2>&1"
( crontab -l 2>/dev/null | grep -v "$INSTALL_PATH" ; echo "$CRON_LINE" ) | crontab -

echo "[✓] Installation complete. Logs: $CRON_LOG"
